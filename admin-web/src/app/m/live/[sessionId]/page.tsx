...
export default function LiveFullscreen({ params }: { params: Promise<{ sessionId: string }> }) {
    ...
    if (soundOn && prevSec != null) {
      ...
      if (prevSec > 0 && sec === 0) {
        ...
        if (blindUpFiredCycleRef.current !== cycleKey) {
          ...
          // playBlindUp/advance 호출 X — lib 단일 경로
        }
      }
    }
    prevSecRef.current = sec;
  ...
  // 블라인드업 백업 — 서버 currentLevel 변경 시점에 1회
  useEffect(() => {
    if (!soundOn) return;
  ...
  // 10초 이하 빨강 강조 (running 중일 때만) — BREAK일 땐 위험 강조 X.
  const isWarning = !paused && !onBreak && sec > 0 && sec <= 10 && session.status === 'running';

  // 2026-05-28: 전체화면 landscape/portrait 분기 제거 — 가로 화면 짤림 문제 + 불필요.
  // 일반 세로 스크롤 카드 레이아웃으로 복원.
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white flex flex-col">
      ...
      {/* 거대 카운트다운 — 10초 이내 빨강 + scale pulse. BREAK은 amber.
          2026-05-24: BREAK일 땐 색·라벨 모두 amber로 휴식 인지 강화
          2026-05-28: isWarning 시 animate-pulse → timer-pulse (scale 1.0↔1.1) */}
      <div className="text-center px-4 mt-2">
        <div
          className={`font-mono font-extrabold leading-none ${isWarning ? 'timer-pulse' : ''}`}
          style={{
            fontSize: '96px',
      ...
      {/* timer-pulse: globals.css에 전역 정의 (.timer-pulse) */}
    </div>
  );
}
