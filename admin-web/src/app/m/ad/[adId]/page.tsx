...
export default function AdDetailPage({
    ...
    return (
      <main className="min-h-screen flex items-center justify-center p-6" style={{ background: 'var(--bg)' }}>
        <div className="empty-state w-full max-w-sm">
          ...
          <div>
            <div className="empty-state-title">광고를 찾을 수 없어요</div>
            <div className="empty-state-desc" style={{ marginTop: 6 }}>
              종료되었거나 잘못된 링크일 수 있어요.
            </div>
          </div>
          <Link href="/m" className="btn-brand tap px-5 py-2.5 text-[13px]">
            홈으로
          </Link>
        </div>
      </main>
  ...
  return (
    <main style={{ background: 'var(--bg)', minHeight: '100vh', paddingBottom: 40 }}>
      ...
      >
        <Link href="/m" aria-label="뒤로" className="text-xl px-1 tap" style={{ color: 'var(--text-1)' }}>
          ←
        </Link>
        <div className="section-title" style={{ margin: 0 }}>📢 광고</div>
        <div className="w-6" />
      </header>
